...
class TurtlebotExploration:
    ...
    def __init__(self):
        ...
        self.data = [] #probabilities are in range [0,100], unknown is -1
        self.origin = Pose()
    ...
    def perform_exploration(self):
        rospy.loginfo("Performing 360 degree sweep of the area...")
        #before the exploration starts, make robot perform an initial 360 sweep of the area 
        self.rotate360()

        #rospy.loginfo("width={}, height={}, resolution={}".format(self.info.width, 
        #    self.info.height, self.info.resolution))
        #rospy.loginfo("originX={}, originY={}".format(self.origin.position.x, 
        #    self.origin.position.y))
        
        rospy.loginfo("Calling frontier exploration server and initialising action client...")
        ...
        x_random, y_random = 0.0, 0.0
        while not rospy.is_shutdown():
            x_random, y_random = random.random() * 20 - 10, random.random() * 20 - 10
            x_cell = int(x_random / self.info.resolution + self.info.origin.position.x)
            y_cell = int(y_random / self.info.resolution + self.info.origin.position.y)
            if self.data[y_cell * self.info.width + x_cell] == -1:
                break
        point = Point(x=x_random, y=y_random, z=0.0)
        ...
        marker_array.markers.extend([self.boundary_marker, frontier_marker])
